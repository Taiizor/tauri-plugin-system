...
impl<R: Runtime> System<R> {
    ...
    pub fn os_info(&self) -> crate::Result<OsInfo> {
        ...
        provider.os_info().map_err(|e| crate::Error::Platform(e.to_string()))
    }

    #[cfg(feature = "cpu")]
    pub fn cpu_info(&self) -> crate::Result<CpuInfo> {
        let provider = self.provider.lock().unwrap();
        provider.cpu_info().map_err(|e| crate::Error::Platform(e.to_string()))
    }

    #[cfg(feature = "cpu")]
    pub fn cpu_usage(&self) -> crate::Result<Vec<f64>> {
        let provider = self.provider.lock().unwrap();
        provider.cpu_usage().map_err(|e| crate::Error::Platform(e.to_string()))
    }

    #[cfg(feature = "memory")]
    pub fn memory_info(&self) -> crate::Result<MemoryInfo> {
        let provider = self.provider.lock().unwrap();
        provider.memory_info().map_err(|e| crate::Error::Platform(e.to_string()))
    }

    #[cfg(feature = "disk")]
    pub fn disk_info(&self) -> crate::Result<Vec<DiskInfo>> {
        let provider = self.provider.lock().unwrap();
        provider.disk_info().map_err(|e| crate::Error::Platform(e.to_string()))
    }

    #[cfg(feature = "gpu")]
    pub fn gpu_info(&self) -> crate::Result<Vec<GpuInfo>> {
        let provider = self.provider.lock().unwrap();
        provider.gpu_info().map_err(|e| crate::Error::Platform(e.to_string()))
    }

    #[cfg(feature = "battery")]
    pub fn battery_info(&self) -> crate::Result<Option<BatteryInfo>> {
        let provider = self.provider.lock().unwrap();
        provider.battery_info().map_err(|e| crate::Error::Platform(e.to_string()))
    }

    #[cfg(feature = "network")]
    pub fn network_info(&self) -> crate::Result<Vec<NetworkInfo>> {
        let provider = self.provider.lock().unwrap();
        provider.network_info().map_err(|e| crate::Error::Platform(e.to_string()))
    }

    #[cfg(feature = "thermal")]
    pub fn thermal_info(&self) -> crate::Result<Vec<ThermalInfo>> {
        let provider = self.provider.lock().unwrap();
        provider.thermal_info().map_err(|e| crate::Error::Platform(e.to_string()))
    }

    #[cfg(feature = "display")]
    pub fn display_info(&self) -> crate::Result<Vec<DisplayInfo>> {
        let provider = self.provider.lock().unwrap();
        provider.display_info().map_err(|e| crate::Error::Platform(e.to_string()))
    }
